...
class JerusalemDirectionViewController: UIViewController, CLLocationManagerDelegate, MKMapViewDelegate {
    ...
    @IBAction func info(_ sender: UIButton) {
        ...
        if Locale.isHebrewLocale() {
            longInfoMessage = "בעת תפילת העמידה יש לכוון את עצמו לכיוון ארץ ישראל, ירושלים, הר הבית ומקום קודש הקודשים (ברכות ל ע״א, על פי מלכים א׳ ח:לה–מח ודברי הימים ב׳ ו:לב). קביעת הכיוון נעשית באמצעות שיטת \"קו הרום\" (Rhumb Line), אשר \"מציירת\" קו ממיקומו של האדם לנקודה מסוימת (ובמקרה שלנו – הר הבית). זו הייתה השיטה המועדפת על מרן זצ״ל כאשר נסע לאמריקה. כאשר המעגל הופך לירוק, סימן שאתה פונה לכיוון האידיאלי לעבר הר הבית. יש לזכור שגם אם ארון הקודש בבית הכנסת שבו אתה נמצא אינו ממוקם נכון, אין להיפרד מן הציבור; אלא יש לכוון רק את הראש לכיוון הנכון."
        } else {
            longInfoMessage = "When praying the Amidah, one should direct himself towards Israel, Jerusalem, the temple mount and the Kodesh Hakodashim (Holy of Holies) area (Berakhot Bavli 30a, based on Kings I 8:35–48 & Chronicles II 6:32). Determining which direction that is in can be done through the \"Rhumb line\" method, which \"draws\" a line from one\'s location to a specific point (which in our case, would be the Temple mount). This was the preferred method of Maran ZT\"L while he was traveling to America. Once the circle turns green, you are facing the ideal direction towards the temple mount. Please keep in mind that even if the Aron in the synagogue that you are in is misplaced, you should not separate from the others. Rather, you should only adjust your head to the right direction."
        }
