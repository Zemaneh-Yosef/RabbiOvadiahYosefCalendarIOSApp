    ...
    func getFullMessage() -> String {
        ...
        if title == zmanimNames.getAchilatChametzString() {
            if Locale.isHebrewLocale() {
                ...
                "elevation included if wanted. Since Chametz is a mitzvah from the torah, we are stringent and we use the Magen Avraham's time to " +
        ...
        if title == zmanimNames.getBrachotShmaString() {
            if Locale.isHebrewLocale() {
                return "זהו הזמן האחרון בו ניתן לאומר ברכות שמע על פי הגר\"א (הגאון רבנו אליהו). בכל זאת, אדם עדיין יכול לאמר פסוקי דזמרה ועמידה עד חצות.  הגר\"א מחשב את הזמן הזה כ-4 שעות זמניות לאחר הזריחה (עם גובה בחשבונות). הגר\"א מחלק את הזמן בין הזריחה והשקיעה (עם גובה בחשבונות) ל-12 חלקים שווים, וכך מתקבלת שעה זמנית אחת."
            } else {
                return "This is the latest time a person can say the Brachot Shema according to the GR\"A. However, a person can still say " +
                "Pisukei D'Zimra and Amidah until Chatzot.\n\n" +
                "The GR\"A calculates this time as 4 zmaniyot/seasonal hours after sunrise (elevation included if wanted). " +
                "The GR\"A calculates a zmaniyot/seasonal hour by taking the time between sunrise and sunset (elevation included if wanted) and " +
        ...
        if title == zmanimNames.getSunsetString() {
            if Locale.isHebrewLocale() {
                ...
                "horizon while setting (elevation included if wanted).\n\n" +
                "Immediately after the sun sets, Bein Hashmashot/twilight starts according to the Geonim, however, according to Rabbenu Tam " +
        ...
        if title == zmanimNames.getTzaitHacochavimString() + " " + zmanimNames.getLChumraString() {
            if Locale.isHebrewLocale() {
                return "זמן זה מחושב כ-20 דקות לאחר שקיעת החמה (כולל השפעת הגובה).  זמן זה חשוב עבור תעניות והכרעה מתי לעשות ברית מילה. מלבד זאת, אין להשתמש בו לשום דבר אחר, כמו הזמן האחרון למנחה. זמן זה מוצג באפור בשבת וביום טוב (בהתאם להמלצת רבותינו) כדי שלא לגרום לאנשים לחשוב ששבת או יום טוב מסתיימים בזמן זה. מחוץ לארץ ישראל, זמן זה מחושב על ידי מציאת מספר הדקות שבין השקיעה לבין 5.075 מעלות מתחת לאופק ביום שבו הזריחה והשקיעה שוות, ולאחר מכן מוסיפים את מספר הדקות הזמניות הזה לשקיעה כדי לקבל את זמן צאת הכוכבים. אנו משתמשים ב-5.075 מעלות מתחת לאופק משום שזה הזמן שבו עוברות 20 דקות לאחר השקיעה בארץ ישראל."
            } else {
                return "This time is calculated as 20 minutes after sunset (elevation included if wanted).\n\n" +
        ...
        if title.contains(zmanimNames.getRTString()) {
            if Locale.isHebrewLocale() {
                return "זמן זה הוא זמן צאת הכוכבים לפי שיטת רבנו תם.\n\nצאת הכוכבים הוא הזמן שבו היום ההלכתי הבא מתחיל לאחר סיום בין השמשות.\n\nלפי רבנו תם, 72 הדקות הללו מורכבות משני חלקים: החלק הראשון הוא 58 וחצי דקות עד לשקיעה השנייה (ראה פסחים צ\"ד ע\"א ותוספות שם). לאחר השקיעה השנייה ישנן עוד 13.5 דקות עד לצאת הכוכבים.\n\nזמן זה מחושב כ־72 דקות זמניות לאחר השקיעה (כולל התחשבות בגובה המקום). עם זאת, מחוץ לארץ ישראל, מחשבים זמן זה על פי המרחק בין השקיעה לבין הרגע שבו השמש נמצאת 16.4 מעלות מתחת לאופק ביום השוויון (כאשר יש 12 שעות בין הזריחה לשקיעה), ומוסיפים את כמות הדקות הזו לזמן השקיעה. במקומות צפוניים בעולם, התאמה זו תגרום לזמן רבנו תם להיות מאוחר מאוד, ולכן אנו מגבילים את הזמן המרבי ל־72 דקות רגילות. הדבר מבוסס על ילקוט יוסף סימן רנ\"ג סעיף ו\' ועל הלכה ברורה חלק ט\"ו עמוד 42.\n\nהגר״א מחשב שעה זמנית על ידי לקיחת הזמן שבין הזריחה לשקיעה (כולל התחשבות בגובה המקום) וחלוקתו ל־12 חלקים שווים. לאחר מכן מחלקים אחד מאותם 12 חלקים ל־60 כדי לקבל דקה זמנית לצורך חישוב 72 דקות."
            } else {
                return "This time is Tzeit/Nightfall according to Rabbenu Tam.\n\nTzeit/Nightfall is the time when the next halachic day starts after Bein Hashmashot/twilight finishes.\n\nAccording to Rabbenu Tam, these 72 minutes are made up of 2 parts. The first part is 58 and a half minutes until the second sunset (see Pesachim 94a and Tosafot there). After the second sunset, there are an additional 13.5 minutes until Tzeit/Nightfall.\n\nThis time is calculated as 72 zmaniyot/seasonal minutes after sunset (elevation included). However, outside of Israel, this time is calculated by finding out the amount of minutes between sunset and when the sun is 16.4 degrees below the horizon on the equinox (when there is 12 hours between sunrise and sunset) and adding that amount of minutes to sunset. In more northern places in the world, this adjustment will make the time for Rabbenu Tam extremely late, therefore, we limit the maximum time limit for Rabbenu Tam to 72 regular minutes. This is based on Yalkut Yosef Siman 293:6 and Halacha Berurah 15 Page 42.\n\nThe GR\"A calculates a zmaniyot/seasonal hour by taking the time between sunrise and sunset (elevation included) and divides it into 12 equal parts. Then we divide one of those 12 parts into 60 to get a zmaniyot/seasonal minute in order to calculate 72 minutes."
            }
        }
        if title == zmanimNames.getChatzotLaylaString() {
            if Locale.isHebrewLocale() {
                return "זהו זמן אמצע הלילה ההלכתי, כאשר השמש נמצאת בדיוק באמצע השמיים מתחת לנו.  מומלץ לסיים את מלווה מלכא לפני זמן זה.  זמן זה מחושב כ-6 שעות זמניות לאחר השקיעה. הגר\"א מחשב שעה זמנית על ידי חלוקת הזמן בין השקיעה והזריחה (עם גובה בחשבונות) ל-12 חלקים שווים."
            } else {
                ...
                "The GR\"A calculates a zmaniyot/seasonal hour by taking the time between sunset and sunrise (elevation included if wanted) and " +
                "divides it into 12 equal parts."
            }
        }
        if title.contains("וּלְכַפָּרַת פֶּשַׁע") {
